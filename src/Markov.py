#!/usr/bin/python3
#
# Based on https://github.com/techlover10/PyTransitionMatrix
...
class TransitionMatrix:
    ...
    def add_transition(self, prev_state, next_state):
        if prev_state in self.matrix.keys():
            self.matrix[prev_state][next_state] = self.matrix[prev_state].get(next_state, 0) + 1
        else:
    ...
    def get_next_outcome(self):
        rand = random.random()
        if self.curr_state not in self.choice_matrix:
